...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType ={
    val first_col = StructField(columnNames.head, StringType, nullable = false)
    val other_col = columnNames.tail.map(field => StructField(field, DoubleType, nullable = false))
    StructType(first_col::other_col)
  }
  ...
  def classifiedFirst(columnNames: List[String], primary_needs: List[Column], working: List[Column], other_activities: List[Column]): (List[Column], List[Column], List[Column]) =
    columnNames match {
      case Nil => (primary_needs, working, other_activities)

      case x::xs =>
        if (x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
          classifiedFirst(xs,  col(x)::primary_needs, working, other_activities)
        else if (x.startsWith("t05") || x.startsWith("t1805"))
          classifiedFirst(xs, primary_needs, col(x)::working, other_activities)
          ...
          classifiedFirst(xs, primary_needs, working,  col(x)::other_activities)
        else
          classifiedFirst(xs, primary_needs, working, other_activities)
    }

  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary_needs = List[Column]()
    val working = List[Column]()
    val other_activities = List[Column]()
    classifiedFirst(columnNames, primary_needs, working, other_activities)
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when(df("teage")>=15 && df("teage")<=22, "young").when(df("teage")>=23 && df("teage")<=55, "active").otherwise("elder")

    def sum_udf(columns: List[Column], sum: Column): Column = columns match {
      case Nil => sum
      case x::xs => sum_udf(xs, sum + df(x.toString()))
    }
    val primaryNeedsProjection: Column = (sum_udf(primaryNeedsColumns, lit(0.0d)) / 60.0d).as("primaryNeeds")
