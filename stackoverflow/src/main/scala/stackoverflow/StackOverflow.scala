...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val question = postings.filter(_.postingType == 1).map(posting => (posting.id, posting))
    val answer = for{
      posting <- postings.filter(_.postingType == 2)
      parentId <- posting.parentId} yield (parentId,posting)
    /** join: extract the QID value in the first element of a tuple, obtain 𝚁𝙳𝙳[(𝚀𝙸𝙳, 𝙸𝚝𝚎𝚛𝚊𝚋𝚕𝚎[(𝚀𝚞𝚎𝚜𝚝𝚒𝚘𝚗, 𝙰𝚗𝚜𝚠𝚎𝚛)])] */
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = 100 * vs.count(_._1 == vs.groupBy(_._1).maxBy(_._2.size)._1) / clusterSize.toDouble // percent of the questions in the most common language
